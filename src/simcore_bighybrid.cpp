...
#ifndef BIGHYBRID_SIMCORE_CODE
#define BIGHYBRID_SIMCORE_CODE
...
void master_mrsg (int argc, char *argv[]);
...
static void run_hybrid_simulation (const char* platform_file, const char* deploy_file, const char* bighybrid_config_file, const char* vc_file);
...
{
 	/*OLD msg_error_t  res_bighybrid = MSG_OK; */
    ...
    run_hybrid_simulation (plat, depl, conf, vc_file);

    print_bighybrid_output();
    
    if (/*OLD res_bighybrid == MSG_OK &&*/ job_mrsg.finished == 1 && job_mra.finished == 1)
   		return 0;
    else
 	  	return 1;    
}
 ...
 */
static void run_hybrid_simulation (const char* platform_file, const char* deploy_file, const char* bighybrid_config_file, const char* vc_file_name)
{
    /*OLD msg_error_t  res_bighybrid = MSG_OK; */
    /*NEW*/
...
static void init_mrsg_config (void)
{
    ...
    config_mrsg.workers_mrsg = xbt_new (/*OLD msg_host_t*/ simgrid::s4u::Host*, config_mrsg.mrsg_number_of_workers);
    ...
    for(unsigned int i = 0; i < host_list.size(); i++){
        ...
        for(unsigned int j = 0; j < actor_list.size(); j++){
            ...
            if ( strcmp (actor->get_cname(), "worker_mrsg") == 0 ) { 
                ...
                /* Add the worker's cpu power to the grid total. */
                /*OLD config_mrsg.grid_cpu_power += MSG_host_get_speed (host);*/
...
static void init_job_mra (void)
{
    ...
    job_mra.task_list[MRA_MAP] = xbt_new0 (/*NEW*/bighybrid_task_t*, config_mra.amount_of_tasks_mra[MRA_MAP]);
    ...
    {
	  job_mra.task_list[MRA_MAP][i] = xbt_new0 (/*NEW*/bighybrid_task_t, MAX_SPECULATIVE_COPIES);
    }
    ...
    job_mra.task_list[MRA_REDUCE] = xbt_new0 (/*NEW*/bighybrid_task_t*, config_mra.amount_of_tasks_mra[MRA_REDUCE]);
    ...
    {
	  job_mra.task_list[MRA_REDUCE][i] = xbt_new0 (/*NEW*/bighybrid_task_t, MAX_SPECULATIVE_COPIES);
	  }
	  for (int i = 0; i < config_mra.mra_number_of_workers ; i++)
    {
...
static void init_job_mrsg (void)
{
  ...
  mrsg_task_pid.status[mrsg_wid] = ON;
    ...
    job_mrsg.task_list[MRSG_MAP] = xbt_new0 (/*NEW*/bighybrid_task_t*, config_mrsg.amount_of_tasks_mrsg[MRSG_MAP]);
    for (int i = 0; i < config_mrsg.amount_of_tasks_mrsg[MRSG_MAP]; i++)
	job_mrsg.task_list[MRSG_MAP][i] = xbt_new0 (/*NEW*/bighybrid_task_t, MAX_SPECULATIVE_COPIES);

    job_mrsg.map_output = xbt_new (size_t*, config_mrsg.mrsg_number_of_workers);
    for (int i = 0; i < config_mrsg.mrsg_number_of_workers; i++)
    ...
    job_mrsg.task_list[MRSG_REDUCE] = xbt_new0 (/*NEW*/bighybrid_task_t*, config_mrsg.amount_of_tasks_mrsg[MRSG_REDUCE]);
    for (int i = 0; i < config_mrsg.amount_of_tasks_mrsg[MRSG_REDUCE]; i++)
	job_mrsg.task_list[MRSG_REDUCE][i] = xbt_new0 (/*NEW*/bighybrid_task_t, MAX_SPECULATIVE_COPIES);
...
static void free_mrsg_global_mem (void)
{
    for (int imrsg = 0; imrsg < config_mrsg.mrsg_chunk_count; imrsg++)
    ...
    for (int imrsg = 0; imrsg < config_mrsg.amount_of_tasks_mrsg[MRSG_MAP]; imrsg++)
	    xbt_free_ref (&job_mrsg.task_list[MRSG_MAP][imrsg]);
    xbt_free_ref (&job_mrsg.task_list[MRSG_MAP]);
    for (int imrsg = 0; imrsg < config_mrsg.amount_of_tasks_mrsg[MRSG_REDUCE]; imrsg++)
