...
static int task_time_elapsed_mrsg (/*OLD msg_task_t*/bighybrid_task_t mrsg_task);
...
void master_mrsg (int argc, char* argv[])
{
    ...
    while (job_mrsg.tasks_pending[MRSG_MAP] + job_mrsg.tasks_pending[MRSG_REDUCE] > 0)
    {
        ...
        if (status == MSG_OK)
        OLD*/
        /*NEW*/ msg = receive(MASTER_MRSG_MAILBOX);
        
        if (msg)    /*NEW*/
        {
            //OLD worker_mrsg = MSG_task_get_source (msg);

            /*NEW*/ worker_mrsg = msg->getSource();
    ...
    /*        #ifdef VERBOSE
            ...
            {
                //OLD ti = (mrsg_task_info_t) MSG_task_get_data (msg);
                
                /*NEW*/ ti = (mrsg_task_info_t) msg->getData();
                ...
                if (job_mrsg.task_status[ti->mrsg_phase][ti->mrsg_tid] != T_STATUS_MRSG_DONE)
                {
                    ...
                    if (job_mrsg.tasks_pending[ti->mrsg_phase] <= 0)
                    {
                            ...
                            if(ti->mrsg_phase==MRSG_MAP)
                            {
                                ...
                                /*NEW*/ stats_mrsg.map_time = simgrid::s4u::Engine::get_clock();
                            ...
                            else
                            {
                                ...
                                /*NEW*/ stats_mrsg.reduce_time = simgrid::s4u::Engine::get_clock() - stats_mrsg.map_time;
...
static int task_time_elapsed_mrsg (/*OLD msg_task_t*/bighybrid_task_t mrsg_task)
{
    ...
    //OLD ti = (mrsg_task_info_t) MSG_task_get_data (mrsg_task);
    /*NEW*/ ti = (mrsg_task_info_t) mrsg_task->getData();

    //OLD return (MSG_task_get_bytes_amount (mrsg_task) - MSG_task_get_flops_amount (mrsg_task))
    //       / MSG_host_get_speed (config_mrsg.workers_mrsg[ti->mrsg_wid]);

    /*NEW*/ return (mrsg_task->getFlopsAmount()*(1 - mrsg_task->getRemainingRatio())) 
...
static void set_mrsg_speculative_tasks (/*OLD msg_host_t*/simgrid::s4u::Host* worker_mrsg)
{
    ...
    if (job_mrsg.mrsg_heartbeats[mrsg_wid].slots_av[MRSG_MAP] < config_mrsg.mrsg_slots[MRSG_MAP])
    {
        for (tid = 0; tid < config_mrsg.amount_of_tasks_mrsg[MRSG_MAP]; tid++)
        {
            ...
            {
                //OLD ti = (mrsg_task_info_t) MSG_task_get_data (job_mrsg.task_list[MRSG_MAP][tid][0]);
                /*NEW*/ ti = (mrsg_task_info_t) job_mrsg.task_list[MRSG_MAP][tid][0]->getData();
    ...
    if (job_mrsg.mrsg_heartbeats[mrsg_wid].slots_av[MRSG_REDUCE] < config_mrsg.mrsg_slots[MRSG_REDUCE])
    {
        for (tid = 0; tid < config_mrsg.amount_of_tasks_mrsg[MRSG_REDUCE]; tid++)
        {
            ...
            {
                //OLD ti = (mrsg_task_info_t) MSG_task_get_data (job_mrsg.task_list[MRSG_REDUCE][tid][0]);
                /*NEW*/ ti = (mrsg_task_info_t) job_mrsg.task_list[MRSG_REDUCE][tid][0]->getData();
...
static void send_reduce_to_mrsg_worker (/*OLD msg_host_t*/simgrid::s4u::Host* dest)
{
    ...
    */
    ...
    for (int t = 0; t < config_mrsg.amount_of_tasks_mrsg[MRSG_REDUCE]; t++)
    {
...
static void send_mrsg_task (enum mrsg_phase_e mrsg_phase, size_t tid, size_t data_src, /*OLD msg_host_t*/simgrid::s4u::Host* dest)
{
    ...
    bighybrid_task_t mrsg_task = nullptr;
    ...
    /*NEW*/  mrsg_task = new Task_BigHybrid(SMS_TASK_MRSG, cpu_required, 0.0, (void*) task_info);
    ...
    /*NEW*/
    mrsg_task->setSender(simgrid::s4u::Actor::self());
    mrsg_task->setSource(simgrid::s4u::Host::current());
    ...
    //OLD xbt_assert (MSG_task_send (mrsg_task, mailbox) == MSG_OK, "ERROR SENDING MESSAGE");
