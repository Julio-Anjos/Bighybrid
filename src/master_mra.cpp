...
#ifndef MRA_MASTER_CODE
#define MRA_MASTER_CODE
...
static int task_time_elapsed_mra (/*OLD msg_task_t*/bighybrid_task_t mra_task);
...
static void send_mra_task (enum mra_phase_e mra_phase, size_t mra_tid, size_t data_src, /*OLD msg_host_t*/ simgrid::s4u::Host* dest);
...
void master_mra (int argc, char* argv[])
{
    mra_heartbeat_t  mra_heartbeat;
    /*OLD msg_error_t  	status_mra;*/
    /*OLD msg_host_t   	worker;*/
    /*OLD msg_task_t   	msg_mra = NULL;*/
    ...
    {
        mra_task_ftm[i].mra_ft_phase 						= MRA_MAP;/*NEW*/
        mra_task_ftm[i].mra_ft_wid   						= 0;
        mra_task_ftm[i].mra_ft_task_status 			        = T_STATUS_MRA_PENDING; /*NEW*/
        mra_task_ftm[i].mra_ft_task_id 					    = 0;
        mra_task_ftm[i].mra_ft_msg    					    = 0;
        mra_ftm_done_s[i].mra_ft_task_status 		        = T_STATUS_MRA_PENDING;/*NEW*/
    ...
    int end_everything = 0;
    while (job_mra.tasks_pending[MRA_MAP] + job_mra.tasks_pending[MRA_REDUCE] > 0 && !end_everything)
    {
        /*OLD
        msg_mra = NULL;
        ...
        OLD*/
        /*Define msg*/
        /*NEW*/ msg_mra = mra_receive(MASTER_MRA_MAILBOX);
        ...
        */
        if (msg_mra)
        {
            /*NEW*/ worker = msg_mra->getSource();
            /*NEW*/ mra_wid = get_mra_worker_id (worker);
            ...
            if (mra_message_is (msg_mra, SMS_HEARTBEAT_MRA))
            {
             ...
             XBT_INFO("DEBBUGING - DIST IS 0. %s",MSG_host_get_name (config_mra.workers_mra[mra_wid])); */
               //XBT_INFO("HEARTBEAT_MRA %s",MSG_host_get_name (config_mra.workers_mra[mra_wid]));
                ...
                {
                    /* Remove machine of execution list */ //DIFF
            ...
            {
                /*OLD ti = (mra_task_info_t) MSG_task_get_data (msg_mra);*/
                /*NEW*/ ti = (mra_task_info_t) msg_mra->getData();

                if(ti->mra_phase == MRA_MAP && ti->mra_tid == 560)
                    XBT_INFO("TASK MAP 560 STATUS %d", job_mra.task_status[ti->mra_phase][ti->mra_tid]);

                if (job_mra.task_status[ti->mra_phase][ti->mra_tid] != T_STATUS_MRA_DONE)
                {
                    //if (behavior[ti->mra_wid] == VC_NORMAL) DIFF
    ...
    fclose(dist_b_log); */
                        ...
                        if (job_mra.tasks_pending[ti->mra_phase] <= 0)
                        {
                            ...
                            if(ti->mra_phase==MRA_MAP)
                            {
                                ...
                                /*NEW*/ stats_mra.map_time = simgrid::s4u::Engine::get_clock();
                            ...
                            else
                            {
                                ...
                                /*NEW*/ stats_mra.reduce_time = simgrid::s4u::Engine::get_clock() - stats_mra.map_time;
...
static void mra_ftm_recover (size_t mra_ftm_vc_wid, int total_tasks)
{
 ...
 //  mra_replica_f (totalOwnedChunks);
    ...
    for (unsigned int k=0; k < config_mra.amount_of_tasks_mra[MRA_MAP]; k++)
    {
    ...
    for (unsigned int kr=0; kr < config_mra.amount_of_tasks_mra[MRA_REDUCE]; kr++)
    {
...
static void get_vc_behavior (double control_timestamp, size_t mra_wid, double last_hb_wid_timestamp )
{
 ...
 /*   while (i < config_mra.mra_number_of_workers )
    ...
    for (i=0; i < config_mra.mra_number_of_workers; i++)
    {
        ...
        if( (unsigned int)(control_timestamp - mra_ftm_detec[i].mra_vc_last_hb) <= config_mra.mra_heartbeat_interval )
        {
            //worker active
...
static int is_straggler_mra (enum mra_phase_e mra_phase, /*OLD msg_host_t*/ simgrid::s4u::Host* worker)
{
    ...
    case MRA_MAP:
        ...
        /*NEW*/ if (worker->get_speed() < mra_dfs_dist[mra_wid].avg_task_exec[MRA_MAP] && task_count > 0)
        {
            return 1;
        }
        break;
        ...
        if (is_straggler_mra (MRA_MAP, worker) == 1 )
        {
            if (job_mra.mra_heartbeats[mra_wid].slots_av[MRA_MAP] < config_mra.mra_slots[MRA_MAP])
            {
                for (mra_tid = 0; mra_tid < config_mra.amount_of_tasks_mra[MRA_MAP]; mra_tid++)
                {
                    ...
                    {
                        /*OLD ti = (mra_task_info_t) MSG_task_get_data (job_mra.task_list[MRA_MAP][mra_tid][0]);*/
                        /*NEW*/ ti = (mra_task_info_t) job_mra.task_list[MRA_MAP][mra_tid][0]->getData();
        ...
        if (is_straggler_mra (MRA_REDUCE, worker) == 1 )
        {
            if (job_mra.mra_heartbeats[mra_wid].slots_av[MRA_REDUCE] < config_mra.mra_slots[MRA_REDUCE])
            {
                for (mra_tid = 0; mra_tid < config_mra.amount_of_tasks_mra[MRA_REDUCE]; mra_tid++)
                {
                    ...
                    {
                        /*OLD ti = (mra_task_info_t) MSG_task_get_data (job_mra.task_list[MRA_REDUCE][mra_tid][0]);*/
                        /*NEW*/ ti = (mra_task_info_t) job_mra.task_list[MRA_REDUCE][mra_tid][0]->getData();
...
static void send_map_to_mra_worker (/*OLD msg_host_t*/ simgrid::s4u::Host* dest)
{
    ...
    /*OLD XBT_INFO ("MRA_map %zu assigned to %s %s", mra_tid, MSG_host_get_name (dest), flags);*/
    /*NEW*/ XBT_INFO ("MRA_map %zu assigned to %s %s", mra_tid, dest->get_cname(), flags);
...
static void send_reduce_to_mra_worker (/*OLD msg_host_t*/ simgrid::s4u::Host* dest)
{
    ...
    */
    ...
    /*OLD XBT_INFO ("MRA_reduce %zu assigned to %s %s", mra_tid, MSG_host_get_name (dest), flags);*/
    /*NEW*/ XBT_INFO ("MRA_reduce %zu assigned to %s %s", mra_tid, dest->get_cname(), flags);
...
static void send_mra_task (enum mra_phase_e mra_phase, size_t mra_tid, size_t data_src, /*OLD msg_host_t*/ simgrid::s4u::Host* dest)
{
    ...
    /*NEW*/ mra_task = new Task_BigHybrid(SMS_TASK_MRA, cpu_required, 0.0, (void*) task_info);
    ...
    /*NEW*/ fprintf (tasks_log, "%d_%zu_%d,%s,%zu,%.3f,START,\n", mra_phase, mra_tid, i, (mra_phase==MRA_MAP?"MRA_MAP":"MRA_REDUCE"), mra_wid, simgrid::s4u::Engine::get_clock());
