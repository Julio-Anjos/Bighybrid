...
static void send_mra_data (/*OLD msg_task_t*/bighybrid_task_t msg_mra);
...
void default_mra_dfs_f (char** mra_dfs_matrix, size_t chunks, size_t workers_mra, int replicas)
{
    ...
    */
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD fprintf (log, " %s , ID: %zu \t Cap_Per: %f \t te_ex: %g \t Dist_B: %u \t Soma: %u \t Pre_ex: %g \t Tem_Cor: %g\n",
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD fprintf (log, " %s , ID: %zu \t Dist_Bruta: %u \t Reduces: %u \t Te_exec_Min: %g \t Tarefas_Reduce %u \t Prev_exec_reduce %g \n",
...
void mra_replica_f (int *totalOwnedChunks)
{
    ...
    for (i=0; i < (mra_dist_manage.max_tot_dist + 1); i++)
    {
        if (mra_dist_manage.total_dist[i] == (int) config_mra.amount_of_tasks_mra[MRA_MAP])
        {
          dist_test = 1;
        }
    }
...
void min_max_f(int adjust, int chunk_num, int fault_mode)
{
    ...
    for (owner = 0; owner < config_mra.mra_number_of_workers; owner++)
    {
        ...
        /*OLD fprintf (log, " %s , ID: %zu \t Dist_Recalc: %u \t Soma: %u \t Chunks %u \t Pre_ex= %g \t Tem_Cor= %g\n ",
    ...
    */
    ...
    while ((minimal_task < dist_min) && tot_sum < config_mra.mra_number_of_workers )
    {
        ...
        for (owner = 0; owner < config_mra.mra_number_of_workers ; owner++)
        {
            ...
            /*OLD printf (log, " %s , ID: %zu \t Dist_Recalc: %u \t Soma: %u \t Te_exec = %g \t Pre_ex= %g \t Tem_Cor= %g\n",
...
void data_node_mra (/*OLD int argc, char* argv[]*/)
{
    ...
    /*NEW*/ bighybrid_task_t msg;

    /*OLD sprintf (mailbox, DATANODE_MRA_MAILBOX, get_mra_worker_id (MSG_host_self ()));*/
    /*NEW*/ sprintf (mailbox, DATANODE_MRA_MAILBOX, get_mra_worker_id (simgrid::s4u::Host::current()));
...
static void send_mra_data (/*OLD msg_task_t msg_mra*/ bighybrid_task_t msg)
{
    char         mailbox[MAILBOX_ALIAS_SIZE];
    double       data_size;
    size_t       my_id;
    ...
    /*OLD sprintf (mailbox, TASK_MRA_MAILBOX, get_mra_worker_id (MSG_task_get_source (msg_mra)), MSG_process_get_PID (MSG_task_get_sender (msg_mra)));*/
    ...
    else if (mra_message_is (msg, SMS_GET_INTER_MRA_PAIRS))
    {
        ...
        /*NEW*/send("DATA-IP", 0.0, data_size, NULL, mailbox);
    }
