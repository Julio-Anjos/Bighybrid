...
static void send_mrsg_data (/*OLD msg_task_t*/ bighybrid_task_t msg);
...
void data_node_mrsg (/*int argc, char* argv[]*/)
{
    ...
    {
		/*OLD msg = NULL;*/
		msg = receive (/*OLD &msg,*/ mailbox);
  		if (/*OLD status == MSG_OK*/ msg)
		{
...
static void send_mrsg_data (bighybrid_task_t msg)
{
    char         mailbox[MAILBOX_ALIAS_SIZE];
    double       data_size;
    size_t       my_id;
    ...
    {
		//OLD MSG_task_dsend (MSG_task_create ("DATA-C", 0.0, config_mrsg.mrsg_chunk_size, NULL), mailbox, NULL);
		/*NEW*/ send("DATA-C", 0.0, config_mrsg.mrsg_chunk_size, NULL, mailbox); //async send?
	}
    else if (mrsg_message_is (msg, SMS_GET_INTER_MRSG_PAIRS))
    {
        ...
        /*NEW*/ send("DATA-IP", 0.0, data_size, NULL, mailbox); //async send?
